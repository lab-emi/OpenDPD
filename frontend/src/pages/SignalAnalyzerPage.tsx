...
import { analyzerDefaults, useAnalyzerSources, useAnalyzeSignal, useUploadSignal, type AnalyzerConfig, type AnalyzerRequest, type AnalyzerSource, type AnalyzerSourceInfo, type SignalAnalysis } from '@/api/signalAnalyzer'
...
export function SignalAnalyzerPage() {
  const sources = useAnalyzerSources()
  ...
  return <Analyzer key={params.toString()} sources={sources.data} />
}

function Analyzer({ sources }: { sources: AnalyzerSourceInfo[] }) {
  const [params] = useSearchParams()
  const qc = useQueryClient()
  const [previous] = useState(() => qc.getQueryData<{ request: AnalyzerRequest; result: SignalAnalysis }>(['signal-analyzer-last']))
  const [selection, setSelection] = useState(() => params.get('source') ? `${params.get('kind')}:${params.get('source')}:${params.get('role') ?? 'input'}` : previous?.request.source ? sourceKey(previous.request.source) : sources[0] ? sourceKey(sources[0].source) : '')
  ...
  const available = uploaded && !sources.some(s => sourceKey(s.source) === sourceKey(uploaded.source)) ? [...sources, uploaded] : sources
  const selected = available.find(s => sourceKey(s.source) === selection)
  ...
  return <Stack spacing={2.5} sx={{ maxWidth: 1600, mx: 'auto' }}>
    <Stack direction="row" useFlexGap sx={{ justifyContent: 'space-between', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
      ...
      <Button component="label" variant="contained" startIcon={<UploadFileIcon />} disabled={pending}>{t('analyzer.upload')}<input type="file" hidden accept=".csv,text/csv" data-testid="analyzer-upload" onChange={e => { const file = e.target.files?.[0]; if (file) upload.mutate(file, { onSuccess: info => { setUploaded(info); choose(sourceKey(info.source), info) } }); e.target.value = '' }} /></Button>
    </Stack>
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'minmax(0,1fr)', lg: '330px minmax(0,1fr)' }, gap: 2.5, alignItems: 'start' }}>
      <Stack spacing={1.5} component="fieldset" disabled={pending} sx={{ m: 0, p: 0, border: 0, minWidth: 0 }}>
        <Paper sx={{ p: 2 }}><Stack spacing={2}>
          <Typography variant="h2">{t('analyzer.source')}</Typography>
          <TextField select fullWidth label={t('analyzer.source')} value={selected ? selection : ''} onChange={e => choose(e.target.value)}>{available.map(item => <MenuItem key={sourceKey(item.source)} value={sourceKey(item.source)}>{item.label}</MenuItem>)}</TextField>
          {selection && !selected && <Alert severity="warning">{t('analyzer.sourceMissing')}</Alert>}
          <Typography variant="body2" color="text.secondary">{t('analyzer.csvHelp')}</Typography>
