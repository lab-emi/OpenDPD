...
test('all dataset entry buttons are grey and inert; built-in examples remain available', async () => {
  ...
  expect(within(guide).getByRole('button', { name: 'Upload CSV · Coming soon' })).toBeDisabled()
  expect(within(guide).getByRole('button', { name: 'Use an existing dataset' })).toBeEnabled()
  const actions = within(guide).getAllByRole('button')
  expect(actions.slice(0, 3).map(button => button.textContent)).toEqual(['Signal Generator', 'Use an existing dataset', 'Upload CSV · Coming soon'])
  expect(guide.querySelectorAll('.MuiButton-contained')).toHaveLength(1)
  expect(actions[0]).toHaveClass('MuiButton-contained')
