...
export function WorkflowProgress() {
  ...
  const steps = [
    ...
    { key: 'output', label: t('paFlow.output'), symbol: 'y[n]', done: supplied || !!state.simulationId, route: supplied ? datasetRoute : '/pa-library' },
    { key: 'paired', label: t('paFlow.paired'), symbol: '(x, y)', done: pair, route: datasetRoute },
  ...
  const active = pathname === '/signal-generator' ? 0 : pathname === '/pa-library' ? (state.simulationId ? 2 : 1)
    : pathname.startsWith('/datasets') ? 3 : new URLSearchParams(search).get('task')?.includes('dpd') ? 5
      : pathname.startsWith('/experiments') || pathname.startsWith('/runs') ? (state.dpdRunId ? 5 : 4) : -1
  const currentStep = steps[active] ?? steps.find(step => !step.done) ?? steps[5]!
  const open = (index: number) => {
    setExpanded(false)
    const step = steps[index]!
    if (index >= 4 && !pair) return
    navigate(step.route)
  }
  const diagram = (large: boolean) => {
    const width = 990, height = large ? 350 : 86
    const positions = large ? [[90, 91], [300, 91], [510, 91], [735, 91], [280, 258], [680, 258]]
      : [[60, 43], [206, 43], [353, 43], [500, 43], [719, 43], [920, 43]]
    return <svg viewBox={'0 0 ' + width + ' ' + height} role="group" aria-label={t('paFlow.title')}
      ...
      {steps.slice(1).map((_, index) => {
        ...
        const path = large && index === 3 ? 'M 778 91 H 955 V 167 H 190 V 258 H 235'
          : 'M ' + (x1!+44) + ' ' + y1 + ' H ' + (x2!-45)
      ...
      {steps.map((step, index) => {
        ...
        return <g key={step.key} role="button" tabIndex={enabled ? 0 : -1} aria-label={step.label + ' · ' + t(step.done ? 'paFlow.complete' : 'paFlow.pending')}
          ...
          {index === 1 || index === 4 ? <polygon points={(x!-32) + ',' + (y!-20) + ' ' + (x!-32) + ',' + (y!+20) + ' ' + (x!+39) + ',' + y}
            fill={fill} stroke={color} strokeWidth="1.5" /> : <>
            {index === 3 && <rect x={x!-35} y={y!-21} width="76" height="34" rx="5" fill={fill} stroke={color} opacity=".6" />}
            <rect x={x!-40} y={y!-16} width="80" height="34" rx={index === 5 ? 4 : 7} fill={fill} stroke={color} strokeWidth="1.5" />
          </>}
          <text x={x!-(index === 1 || index === 4 ? 7 : 0)} y={y!+5} textAnchor="middle" fill={color} fontFamily="ui-monospace, monospace" fontWeight="700" fontSize="15">{step.symbol}</text>
  ...
  return <>
    <Paper variant="outlined" sx={{ px: 1.5, py: .5, mb: 2, borderRadius: 2, position: 'sticky', top: 64, zIndex: 5 }} data-testid="studio-workflow">
      <Stack direction="row" sx={{ alignItems: 'center', gap: 1 }}>
        ...
        <Typography variant="body2" noWrap sx={{ flex: 1, display: { xs: 'block', md: 'none' } }}>{currentStep.label} · {steps.filter(step => step.done).length}/6</Typography>
