...
export function DatasetGuide({ onCsv, onBuiltin, onGenerator, onSkip, customDatasets = false }: { onCsv: () => void; onBuiltin: () => void; onGenerator: () => void; onSkip: () => void; customDatasets?: boolean }) {
  ...
  return <Dialog open onClose={onSkip} fullWidth maxWidth="sm" aria-labelledby="dataset-guide-title" slotProps={{ backdrop: GUIDE_BACKDROP, paper: { sx: { borderRadius: 3, boxShadow: `0 24px 90px ${colors.backdrop}` } } }}>
    ...
    <DialogContent>
      <Typography color="text.secondary" sx={{ mb: 2.5 }}>{t('generator.guideIntro')}</Typography>
      <Stack spacing={1.5} sx={{ mb: 3 }}>
        {(['generator.guideChoose', 'generator.guideInspect', 'generator.guideTrain'] as const).map((key, index) => <Stack key={key} direction="row" spacing={1.5} sx={{ alignItems: 'center' }}>
      ...
      <Stack spacing={1.25}>
        <Button variant="contained" size="large" startIcon={<GraphicEqIcon />} endIcon={<ArrowForwardIcon />} onClick={onGenerator} sx={{ minHeight: 58 }}>{t('generator.title')}</Button>
        <Button variant="outlined" size="large" startIcon={<DatasetIcon />} onClick={onBuiltin}>{t('generator.existing')}</Button>
        <Button variant="outlined" disabled={!customDatasets} size="large" startIcon={<UploadFileIcon />} onClick={onCsv}>{t('generator.upload')}{!customDatasets && ` · ${t('common.comingSoon')}`}</Button>
      </Stack>
    </DialogContent>
