...
export const taskGroup = (task: string) => task === 'train_dpd' || task === 'run_dpd' ? 'dpd' : 'pa'
...
export function ExperimentTasks({ active, dataset, version, compact = false }: { active?: string; dataset?: string; version?: string; compact?: boolean }) {
  ...
  return <Box component="nav" aria-label={t('tasks.choose')} sx={{ display: 'grid', gridTemplateColumns: { xs: compact ? 'repeat(2, minmax(0, 1fr))' : 'minmax(0, 1fr)', sm: 'repeat(2, minmax(0, 1fr))' }, gap: 1.25, '& > a': { minWidth: 0, overflowWrap: 'anywhere' } }}>
    {(['train_pa', 'train_dpd'] as const).map((task, index) => {
      const params = new URLSearchParams({ task })
      if (dataset) params.set('dataset', dataset)
      if (version) params.set('version', version)
      const selected = !!active && taskGroup(active) === taskGroup(task)
      const Icon = ModelTrainingIcon
      return <ButtonBase component={RouterLink} to={`/experiments/new?${params}`} key={task} aria-current={selected ? 'page' : undefined} sx={{ p: compact ? 1.5 : 2, display: 'block', textAlign: 'left', height: '100%', border: 1, borderColor: selected ? 'primary.main' : 'divider', borderRadius: 1.5, bgcolor: selected ? colors.selected : 'background.paper', boxShadow: selected ? `inset 0 -3px ${colors.primary}` : 'none', '&:hover': { borderColor: 'primary.main', bgcolor: colors.surfaceMuted }, '&.Mui-focusVisible': { outline: `3px solid ${colors.primary}`, outlineOffset: 2 } }}>
        ...
        <Typography sx={{ fontSize: compact ? 13 : 15, fontWeight: 700 }}>{t(`modelWorkflow.${taskGroup(task)}`)}</Typography>
        {!compact && <Typography variant="body2" color="text.secondary" sx={{ mt: .75 }}>{t(`modelWorkflow.${taskGroup(task)}Help`)}</Typography>}
      </ButtonBase>
    })}
  </Box>
}
