...
| Leaking secrets in URLs/logs | bootstrap token | process-lifetime local token exchanged via `GET /bootstrap` or the manual bootstrap form, then removed from the address bar by redirect; reuse supports reopening Studio and each exchange creates a new session; never logged by the app; API responses are `Cache-Control: no-store` | `test_studio_api.py::test_bootstrap_redirect_strips_token`, `test_every_response_carries_the_security_headers` |
