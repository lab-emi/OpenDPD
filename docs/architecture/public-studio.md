...
CSV dataset uploads use a dedicated authenticated, rate-limited endpoint. Files must have a `.csv` suffix and contain UTF-8 comma-separated paired numeric samples: two complex columns or four real I/Q columns, with an optional header. Limits are 25 MiB, 1,000,000 sample rows and four accepted uploads per session. The server scans every row in a private quarantine folder before exposing any preview or import reference. Binary/control bytes, invalid encoding, oversized lines, broken CSV, non-finite samples and formulas/code in data cells are rejected; failed and interrupted partial uploads are deleted immediately. Successful uploads receive a random identifier and SHA-256 proof bound to their session. Import and preview routes only accept those validated sources, and imports reserve workspace capacity before materialisation. Users may correct their own dataset metadata; FFT size (65,536), subchannel count (32), rates and text lengths are bounded, and reference-waveform bindings are forbidden. This validates a restricted data format; it does not execute files or claim antivirus certification.
...
## Signal inspection (2.2.9)

The explicit public allowlist includes signal-source listing, analysis and numeric CSV upload. Signal uploads use the same trusted-origin, bearer, body streaming, timeout and quarantine boundary as paired CSV uploads, with a separate validated array store. Every field is checked before publication; limit 25 MiB, eight columns, two million numeric fields and one million rows. Storage is reserved before upload, and each workspace retains at most 16 such signals.

Analysis uses at most one million selected samples, a 16,384-point PSD and a bounded 256 × 128 spectrogram display. Requests occupy the shared heavy-computation limit and workspace mutation mutex, with a per-network rate limit. They do not reserve disk because analysis itself writes no files. Source identifiers resolve only within the authenticated workspace; unavailable-source errors disclose no host paths. Status and cancellation remain usable while numeric capacity is full.

## Maintenance hardening in 2.2.10

Install `opendpd-tunnel-guard.service` and `/etc/opendpd-web/tunnel-guard.sh` before starting the dashboard-managed connector. The script needs nftables and the dedicated `opendpd-tunnel` identity. It atomically replaces only the `inet opendpd_tunnel` table. New loopback connections from that identity may reach TCP `127.0.0.1:18765`; SSH, metrics and other loopback services are refused. Internet tunnel connections and other host identities retain their existing firewall policy. Test the allowed API connection and refused SSH/metrics connections after installation.

The GPU service retains rootful Podman on this installation, with an explicit capability set for OCI mounts, namespace setup and ownership changes. `NoNewPrivileges`, loopback-only IP access, restricted address families, protected kernel settings and inaccessible VM/tunnel secrets also apply to the agent. Containers retain their separate unprivileged UID, empty capability set, read-only root and disabled network. A startup check rejects writable source trees, symlinks and exposed GPU credentials. This reduces the rootful agent's reach; it does not equate to a rootless deployment.

`requirements-gpu.lock` fixes the Python additions to the digest-pinned CUDA image; installation requires hashes. `requirements-vm.lock` records the Python 3.14 Linux CPU environment, including its CPU tensor wheel. Build a wheelhouse on a connected staging host with those exact hashes, then install offline with `uv pip sync --python /opt/opendpd-venv/bin/python --no-index --find-links /opt/wheelhouse --require-hashes requirements-vm.lock`. The source package is installed separately with `--no-deps --no-build-isolation`. Record the source commit and final OCI image digest for each deployment. OS security updates must be rebuilt and tested separately.
