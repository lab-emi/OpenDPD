...
| Worker ↔ server | Worker emits JSON-lines events on its stdout pipe; stderr/log go to `logs/worker.log` | No shared memory, no second database writer, cross-platform. |
