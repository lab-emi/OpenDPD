...
| `GET /models` (`ModelInfo.execution_semantics`, `weights_from`) | S18: streaming variants (`gru_stream`, `gmp_stream`) carry `execution_semantics: streaming_stateful` and name in `weights_from` the offline key whose weights they execute; `POST /runs` with such a model is an `evaluate_pa` / `run_dpd` run bound to a finished run of that key (training it is refused); `EvaluationResult.execution` records chunk, look-ahead (samples and seconds), history, measured warm-up and the chunk-consistency check; the comparison key keeps streaming and offline results apart (`docs/architecture/streaming.md`) |
| `POST /deploy/exports` (`{run_id}`) | S19: a `fixed-point-v1` deployment package for a succeeded GRU run under `<workspace>/exports` (quantised weights, six golden vectors with the state trace, the C99 reference compiled and verified bit for bit, the labelled report); `422 deploy_unsupported` with the reason for models whose `export_formats` lack the format; downloaded through `GET /exports/{id}` (`docs/protocols/fixed-point-v1.md`) |
