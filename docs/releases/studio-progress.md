...
| S11 result comparison, visualisation & reproducible packages | done except the G1 human-gated items below | `abaf8af` (S11a) + S11b | plots-v1 data with fixed budgets, per-epoch history, comparison under one profile with explicit incompatibilities (incl. PA surrogate, reference gain, execution semantics), full/share packages with hash-verified import, reports bound to stored results, `opendpd export/import/report`, Export panel + Import package in the GUI |
