...
| Real-browser evidence | pending: Playwright J3 covers export → import → open run on the mock API; a pass against `opendpd gui` on a real workspace is recorded in the S11 closure row below when done |
| G1 gate: import → diagnose → PA → DPD → evaluate → export → CLI re-evaluation on real data | partly: every link is exercised by automated tests on the measured built-in data (`test_cli_run.py`, `test_packages.py`, headless J2 in S07); one scripted end-to-end pass is recorded in the closure row below when done. **Pending human:** at least one internal user not involved in the implementation completes the flow independently — cannot be produced by the implementation |
