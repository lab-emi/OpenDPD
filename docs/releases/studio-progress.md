...
| S08 versioned metrics & evaluation evidence | done | — | `opendpd/core/metrics` registry (`legacy-opendpd-v1` frozen, `general-spectral-v1`), analytic reference tests, results re-scored from the best checkpoint under every profile, profile switcher and definitions in the GUI |
