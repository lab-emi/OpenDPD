...
| `execution.num_threads` accepted by the schema but applied nowhere | A declared knob that did nothing. | `apply_thread_budget` in the shared executor applies it on every path (`test_thread_budget_is_applied_by_the_executor`); the harness measures overhead with torch's default budget (the target figure) and again with an explicit equal budget as a diagnostic. |
