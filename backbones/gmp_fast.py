...
class GMP(nn.Module):
    ...
    def reset_parameters(self):
        for name, param in self.named_parameters():
            if 'A_kl' in name or 'B_klm' in name:
    ...
    def forward(self, x, h_0):
        ...
        x_first_term = torch.cat((zero_pad, x_complex), dim=1)  # Dim: (batch_size, frame_length + L - 1)
        ...
        x_second_term = torch.cat((zero_pad, x_complex), dim=1)  # Dim: (batch_size, frame_length + L + M - 1)
        ...
        # Merge Terms
        sum_terms = sum_first_term + sum_second_term
