...
def run_dpd_config(dataset_id: str, dpd_run_id: str, *, device: str = "cpu") -> ExperimentConfig:
    """A run_dpd task for an existing DPD run (model/PA are bound from that run)."""
    return ExperimentConfig(
        task=TaskType.run_dpd, name=f"run_dpd of {dpd_run_id}", dataset=DatasetRef(id=dataset_id),
        model=ModelSpec(key="gru"), evaluation=EvaluationConfig(evidence_type=EvidenceType.dpd_surrogate),
