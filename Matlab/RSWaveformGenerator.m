...
classdef RSWaveformGenerator < handle
    ...
    methods
        ...
        function upload_wave(obj, ftp_host, ftp_username, ftp_password, ftp_remote_dir)
            ...
            %   ftp_username - caller-supplied FTP username
            %   ftp_password - caller-supplied FTP password
            ...
            if nargin < 4 || isempty(ftp_username) || isempty(ftp_password)
                error('Explicit instrument credentials are required');
            end
            if nargin < 5
                ftp_remote_dir = '/user';
            end
